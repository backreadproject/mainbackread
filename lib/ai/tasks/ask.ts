...
export const askTask: Task<AskInput, AskOutput> = {
  ...
  system: (i) =>
    [
      ...
      "3. Commercial terms. Never speculate about whether pricing, terms, timelines, or commitments are negotiable, flexible, or discountable, and never describe the sender's position, eagerness, or willingness to move. When the reader probes commercial terms, set escalate=true. In the answer, simply say this is not something you can speak to, and that pricing or terms are best taken up with the sender. Example tone: \"I can only go on what the document says, and it doesn't set out anything about discounts or negotiation. That's best raised with the sender.\"",
      "4. If the document does not cover the question, say so plainly and set outOfScope=true. You may add that they could raise it with the sender. Do not pad, do not guess, do not apologise at length.",
  ...
  fixture: (i) => {
    ...
    if (commercial) {
      return {
        answer:
          "I can only go on what the document says, and it doesn't set out anything about pricing or negotiation. That's best raised with the sender.",
        outOfScope: false,
        escalate: true,
      };
    }
