...
export async function extractText(
  ...
  if (kind === "docx") {
    ...
    return { text, method: "docx", needsPageOcr: false, chars: text.length };
  }

  if (kind === "image") {
    const text = await extractImage(bytes, mime, name);
    return { text, method: "image-ocr", needsPageOcr: false, chars: text.length };
  }
  ...
  if (kind === "pdf") {
    ...
    if (contentLen < SCANNED_PDF_THRESHOLD) {
      ...
      return { text: "", method: "empty", needsPageOcr: true, chars: 0 };
    }
    return { text, method: "pdf-text", needsPageOcr: false, chars: text.length };
  }

  // Unknown type: nothing we can do in Phase 1.
  return { text: "", method: "empty", needsPageOcr: false, chars: 0 };
}
