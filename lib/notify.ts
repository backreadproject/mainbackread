...
export function notifyEmail(heading: string, message: string, actionUrl?: string, actionLabel?: string): string {
  const button = actionUrl ? `<a href="${actionUrl}" style="display:inline-block;background:#0B7A4B;color:#fff;font-size:15px;font-weight:600;text-decoration:none;padding:12px 24px;border-radius:10px;margin-top:8px;">${actionLabel ?? "Open BackRead"}</a>` : "";
