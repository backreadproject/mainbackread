...
const INK = "#0A0E17", CARD = "#FFFFFF", BLUE = "#1D4ED8", SLATE = "#475569", LINE = "#E7EBF2", RED = "#DC2626", RED_BG = "#FEF2F2", GREEN = "#059669";
  ...
  return (
    <div style={{ fontFamily: INTER, color: INK, minHeight: "100vh" }}>
      ...
      <main style={{ maxWidth: 520, padding: "28px 36px" }}>
        <h1 style={{ fontSize: 26, fontWeight: 600, letterSpacing: "-0.015em", margin: "0 0 24px" }}>Account</h1>
        ...
        <div style={card}>
          ...
          <button onClick={changePassword} disabled={busy || !pw} className="fx-b" style={{ background: BLUE, color: "#fff", border: "none", borderRadius: 10, padding: "11px 18px", fontSize: 14, fontWeight: 400, fontFamily: INTER, opacity: busy || !pw ? 0.45 : 1 }}>
        ...
        <div style={{ ...card, border: "1px solid #FBE3E3" }}>
          ...
          <label style={{ fontSize: 13, color: SLATE, display: "block", marginBottom: 6 }}>Type <span style={{ fontWeight: 400, color: RED }}>delete</span> to confirm</label>
          ...
          <button onClick={deleteAccount} disabled={!canDelete || delBusy} className="fx-b"
            style={{ background: canDelete ? RED : "#F1F5F9", color: canDelete ? "#fff" : SLATE, border: "none", borderRadius: 10, padding: "11px 18px", fontSize: 14, fontWeight: 400, fontFamily: INTER, cursor: canDelete ? "pointer" : "default", opacity: delBusy ? 0.6 : 1 }}>
