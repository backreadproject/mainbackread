...
export default async function OverviewPage() {
  ...
  for (const s of signals) {
    ...
    if (s.kind === "opened") { openedRecipients.add(s.recipient_id); if (recentEvents.length < 6) recentEvents.push({ text: `${who} ${t.opened} ${doc}`, at: s.created_at, kind: "opened" }); }
    if (s.kind === "question") { questionCount++; if (recentEvents.length < 6) { const qq = (s.value && typeof s.value === "object" && "text" in s.value) ? String((s.value as {text:string}).text) : ""; recentEvents.push({ text: `${who} ${t.asked} "${qq}"`, at: s.created_at, kind: "question" }); } }
  }
  const stats = {
