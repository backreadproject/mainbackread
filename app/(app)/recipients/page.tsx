...
export default async function RecipientsPage() {
  ...
  const { data: recipients } = await supabase.from("recipients").select("id, label, share_token, document_id, created_at").order("created_at", { ascending: false });
  ...
  const rows = recs.map((r) => ({ id: r.id, label: r.label, documentTitle: docMap.get(r.document_id) ?? "Untitled", createdAt: r.created_at, opened: opened.has(r.id), questions: qc[r.id] ?? 0 }));
  const stats = { total: recs.length, opened: opened.size, unopened: recs.length - opened.size, questions: totalQ, escalated: totalEsc };
