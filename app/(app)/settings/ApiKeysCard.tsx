  ...
  return (
    <div style={card}>
      ...
      <p style={{ fontSize: 13, color: T.body, margin: "0 0 14px", lineHeight: 1.5 }}>
        Use these keys with Zapier, Make, or your own code. Base URL: <code style={{ fontSize: 12, background: "#F2F4F7", padding: "1px 5px", borderRadius: 4 }}>/api/v1</code>. Send the key as <code style={{ fontSize: 12, background: "#F2F4F7", padding: "1px 5px", borderRadius: 4 }}>Authorization: Bearer rp_...</code>
      </p>
        ...
        <div key={k.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: "10px 0", borderTop: `1px solid ${T.border}` }}>
          ...
          {canManage && (
            <div style={{ display: "flex", gap: 6, flex: "none" }}>
              ...
              <button onClick={async () => { if (await call({ action: "delete", keyId: k.id })) router.refresh(); }} disabled={busy} style={{ ...small, color: "#B42318", borderColor: "#FDA29B" }}>Delete</button>
            </div>
          )}
        </div>
      ))}
      ...
      {canManage && (
        <div style={{ marginTop: 14 }}>
          ...
          <button
            ...
            disabled={busy}
            style={{ background: T.green, color: "#fff", border: "none", borderRadius: T.rBtn, padding: "10px 16px", fontSize: 14, fontWeight: 600, fontFamily: T.font, cursor: "pointer", opacity: busy ? 0.5 : 1 }}>
            Create key
          </button>
        </div>
      )}
