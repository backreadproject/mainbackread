...
function Card({ head, children }: { head?: string; children: React.ReactNode }) {
  return (
    <div style={{ border: "1px solid " + T.border, borderRadius: T.rCard, overflow: "hidden" }}>
      {head && (
        <div style={{ background: T.soft, borderBottom: "1px solid " + T.border, padding: "8px 14px", fontSize: 12, fontWeight: 500, color: T.muted }}>
          {head}
        </div>
      )}
      {children}
...
export default function IcpOutputView({ row, locale, onReanswer }: { row: Row; locale: Locale; onReanswer: () => void }) {
  const c = icpCopy(locale);
  const o = row.output;
  if (!o) return <Blank title={c.noResult} hint={c.noResultHint} />;

  // The model returns the stance as a fixed English key, which is what makes it
  // translatable here. Had it returned display text, this row would be English
  // on a French page and nothing would have caught it.
  const stanceWord: Record<string, string> = {
    signs: c.stanceSigns, champions: c.stanceChampions, blocks: c.stanceBlocks,
  };
    ...
    <div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 14px", alignItems: "center", marginTop: 12, paddingTop: 12, borderTop: "1px solid " + T.border, fontSize: 12.5, color: T.muted }}>
        ...
        <span style={{ marginLeft: "auto" }}>
          <button onClick={onReanswer} style={btn}>{c.reanswer}</button>
        </span>
      </div>
      ...
      {hypothesis && (
        <div style={{ marginTop: 20, borderLeft: "3px solid " + T.amber, padding: "2px 0 2px 14px", maxWidth: 640 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: T.heading, marginBottom: 4 }}>{c.readFirstH}</div>
          <div style={{ fontSize: 13.5, color: T.body, lineHeight: 1.6 }}>{c.readFirstP}</div>
        </div>
      )}

      <Sec h={hypothesis ? c.hHypothesis : c.hDefinition} sub={hypothesis ? undefined : c.sDefinition}>
        <Card>
          {o.definition.map((d, i) => <Rw key={i} k={d.label} v={d.value} last={i === o.definition.length - 1} />)}
        </Card>
      </Sec>

      {o.triggers.length > 0 && (
        <Sec h={c.hTriggers} sub={c.sTriggers}>
          <List items={o.triggers.map((t) => ({ t: t.event, d: t.why }))} />
        </Sec>
      )}
      ...
      {o.committee.length > 0 && (
        <Sec h={c.hCommittee} sub={c.sCommittee}>
          <Card head={c.committeeHead}>
            {o.committee.map((x, i) => (
              <Rw key={i} dot={STANCE_COLOUR[x.stance] ?? T.border}
                k={x.role + " \u2014 " + (stanceWord[x.stance] ?? x.stance)}
                v={x.cares} last={i === o.committee.length - 1} />
            ))}
          </Card>
        </Sec>
      )}
        ...
        {o.find.searchStrings.length > 0 && (
          <div style={{ marginTop: 10 }}>
            {o.find.searchStrings.map((s, i) => (
              <div key={i} style={{ fontFamily: "ui-monospace, Consolas, monospace", fontSize: 12.5, color: T.heading, background: T.soft, border: "1px solid " + T.border, borderRadius: T.rPill, padding: "7px 10px", overflowX: "auto", whiteSpace: "pre", marginTop: i ? 6 : 0 }}>
                {s}
              </div>
            ))}
          </div>
      ...
      {o.disqualifiers.length > 0 && (
        <Sec h={c.hDisq} sub={c.sDisq}>
          <List items={o.disqualifiers.map((d) => ({ t: d.who, d: d.why }))} />
        </Sec>
      )}
      ...
      {o.angles.length > 0 && (
        <Sec h={c.hAngles} sub={c.sAngles}>
          <Card>
            {o.angles.map((a, i) => <Rw key={i} k={a.persona} v={a.lead} last={i === o.angles.length - 1} />)}
          </Card>
        </Sec>
      )}

      {o.test.length > 0 && (
        <Sec h={c.hTest} sub={c.sTest}>
          <List items={o.test.map((t) => ({ t: t.step, d: t.detail }))} />
        </Sec>
      )}
