...
export default function SupportWidget({ surface = "marketing", firstName }: { surface?: "marketing" | "app"; firstName?: string | null }) {
  ...
  const card = { background: "var(--rp-card)", borderRadius: 13, boxShadow: "0 2px 10px rgba(9,30,22,0.07)", padding: 14 };
  ...
  return (
    <>
      <style>{`
        ...
        .rp-sw-row:hover{background:var(--rp-soft)}
      ...
      {open && (
        <div className="rp-sw-panel" style={{ position: "fixed", right: 22, bottom: 88, width: 372, height: "min(560px, calc(100vh - 130px))", background: T.canvas, borderRadius: 16, boxShadow: "0 20px 60px rgba(9,30,22,0.24)", border: `1px solid ${T.border}`, display: "flex", flexDirection: "column", overflow: "hidden", zIndex: 9998, fontFamily: T.font, letterSpacing: T.tracking }}>
            ...
            <div style={{ flex: 1, overflowY: "auto" }}>
              <div style={{ background: T.sidebarGradient, padding: "26px 20px 34px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  ...
                  <button onClick={() => setOpen(false)} aria-label="Close" style={{ background: "rgba(255,255,255,0.12)", border: "none", borderRadius: 7, width: 26, height: 26, color: "var(--rp-on-accent)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <X size={14} strokeWidth={2.2} />
                  </button>
                </div>
                <div style={{ marginTop: 22, color: "var(--rp-on-accent)", fontSize: 23, fontWeight: 700, letterSpacing: T.trackingTight, lineHeight: 1.3 }}>
            ...
            <>
              <div style={{ background: T.sidebarGradient, padding: "15px 18px", flexShrink: 0 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <span style={{ color: "var(--rp-on-accent)", fontSize: 15, fontWeight: 700 }}>Messages</span>
                  <button onClick={() => setOpen(false)} aria-label="Close" style={{ background: "rgba(255,255,255,0.12)", border: "none", borderRadius: 7, width: 26, height: 26, color: "var(--rp-on-accent)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <X size={14} strokeWidth={2.2} />
                  </button>
                </div>
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "14px 14px 6px" }}>
                ...
                {visible.map((m, i) =>
                  m.role === "note" ? (
                    ...
                    <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start", marginBottom: 9 }}>
                      <div style={{ ...bubble, background: m.role === "user" ? T.green : "var(--rp-card)", color: m.role === "user" ? "var(--rp-on-accent)" : T.heading, border: m.role === "user" ? "none" : `1px solid ${T.border}` }}>
                ...
                {waiting && !hasEmail && surface === "marketing" && (
                  <div style={{ background: "var(--rp-card)", border: `1px solid ${T.border}`, borderRadius: 12, padding: 12, marginTop: 6 }}>
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      <button onClick={saveEmail} style={{ background: T.green, color: "var(--rp-on-accent)", border: "none", borderRadius: 8, padding: "8px 12px", fontSize: 13, fontWeight: 600, fontFamily: T.font, cursor: "pointer" }}>Save</button>
                    </div>
                  </div>
                )}
                {err && <div style={{ fontSize: 13, color: "var(--rp-danger-text)", padding: "4px 2px" }}>{err}</div>}
                <div ref={endRef} />
              </div>

              <div style={{ borderTop: `1px solid ${T.border}`, padding: 11, display: "flex", gap: 8, background: "var(--rp-card)", flexShrink: 0 }}>
                ...
                <button onClick={() => send()} disabled={busy || !draft.trim()} aria-label="Send"
                  style={{ background: T.green, color: "var(--rp-on-accent)", border: "none", borderRadius: 20, width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0, opacity: busy || !draft.trim() ? 0.5 : 1 }}>
            ...
            <>
              <div style={{ background: T.sidebarGradient, padding: "15px 18px", flexShrink: 0 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                  <span style={{ color: "var(--rp-on-accent)", fontSize: 15, fontWeight: 700 }}>Help</span>
                  <button onClick={() => setOpen(false)} aria-label="Close" style={{ background: "rgba(255,255,255,0.12)", border: "none", borderRadius: 7, width: 26, height: 26, color: "var(--rp-on-accent)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <X size={14} strokeWidth={2.2} />
                  </button>
                </div>
                <input className="rp-sw-in" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search for help"
                  style={{ width: "100%", boxSizing: "border-box", border: "1px solid rgba(255,255,255,0.16)", borderRadius: 9, padding: "9px 12px", fontSize: 13, fontFamily: T.font, background: "rgba(255,255,255,0.1)", color: "var(--rp-on-accent)" }} />
              </div>
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: "10px 14px 14px" }}>
                {faqHits.length === 0 && (
                  <div style={{ padding: "16px 2px" }}>
                    ...
                    <button onClick={() => { setTab("messages"); setDraft(search); }} style={{ background: T.green, color: "var(--rp-on-accent)", border: "none", borderRadius: T.rBtn, padding: "9px 16px", fontSize: 13, fontWeight: 600, fontFamily: T.font, cursor: "pointer" }}>Ask about this</button>
                  </div>
                )}
                {faqHits.map((f, i) => {
                  ...
                  return (
                    <div key={idx} style={{ background: "var(--rp-card)", borderRadius: 11, marginBottom: 8, boxShadow: "0 1px 4px rgba(9,30,22,0.05)", overflow: "hidden" }}>
          ...
          {/* TABS */}
          <div style={{ display: "flex", borderTop: `1px solid ${T.border}`, background: "var(--rp-card)", flexShrink: 0 }}>
            {([
              ["home", "Home", House],
              ...
              <button key={id} onClick={() => setTab(id)} className="rp-sw-tab"
                ...
                <span style={{ position: "relative", display: "flex" }}>
                  ...
                  {id === "messages" && waiting && <span style={{ position: "absolute", top: -2, right: -4, width: 7, height: 7, borderRadius: 4, background: "var(--rp-danger)" }} />}
                </span>
      ...
      <button onClick={() => setOpen((v) => !v)} className="rp-sw-launch" aria-label="Support"
        ...
        {open ? (
          <X size={21} color="var(--rp-on-accent)" strokeWidth={2.2} />
        ) : (
          <MessageCircle size={22} color="var(--rp-on-accent)" strokeWidth={1.9} />
        )}
      </button>
    </>
  );
