...
export default async function NotFound() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.canvas, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: T.font, letterSpacing: T.tracking, color: T.heading, padding: 40, textAlign: "center" }}>
      ...
      <h1 style={{ fontSize: 30, fontWeight: 700, letterSpacing: T.trackingTight, color: T.heading, margin: "0 0 10px" }}>{C.notFoundH}</h1>
      <p style={{ fontSize: 16, color: T.body, margin: "0 0 26px", maxWidth: 420, lineHeight: 1.55 }}>{C.notFoundB}</p>
      <Link href="/" style={{ background: T.green, color: "var(--rp-on-accent)", fontSize: 15, fontWeight: 600, padding: "11px 22px", borderRadius: T.rBtn, textDecoration: "none", boxShadow: "0 4px 14px rgba(11,122,75,0.22)" }}>{C.takeMeHome}</Link>
    </div>
  );
}
